        ...
        )
{
    ...
    *ppMeshB = NULL;
    ...
    if (cFacesA && cVerticesA)
    {
        ...
        hr  = D3DXCreateMeshFVF(cFacesA, cVerticesA, D3DXMESH_SYSTEMMEM | dw32bit, pMesh->GetFVF(), pDevice, &pMeshA);
        if (FAILED(hr))
            goto e_ExitA;
...
e_ExitA:
    ...
    if (pMeshA != NULL)
    {
        ...
        hr = pMeshA->CloneMeshFVF(optionsA, pMeshA->GetFVF(), pDevice, ppMeshA);
        if (FAILED(hr))
            goto e_Exit;
    ...
    if (cFacesB && cVerticesB)
    {
        ...
        hr  = D3DXCreateMeshFVF(cFacesB, cVerticesB, D3DXMESH_SYSTEMMEM | dw32bit, pMesh->GetFVF(), pDevice, &pMeshB);
        if (FAILED(hr))
            goto e_ExitB;
...
e_ExitB:
    ...
    if (pMeshB != NULL)
    {
        ...
        hr = pMeshB->CloneMeshFVF(optionsB, pMeshB->GetFVF(), pDevice, ppMeshB);
        if (FAILED(hr))
            goto e_Exit;
...
HRESULT CMyD3DApplication::GenerateMesh(SMeshContainer *pmcMesh)
{
    ...
    hr  = pDevice->SetRenderState(D3DRS_SOFTWAREVERTEXPROCESSING, FALSE);
    if (FAILED(hr))
        goto e_Exit;
    ...
    if (m_method == D3DNONINDEXED)
    {
        ...
        if (caps.MaxVertexBlendMatrices == 2)       
        {
            ...
            // split the mesh
            LPD3DXMESH pMeshHW = NULL;
            hr  = SplitMesh(pmcMesh->pMesh, pmcMesh->iAttrSplit, rgiAdjacency, D3DXMESH_WRITEONLY,  pmcMesh->pMesh->GetOptions() | D3DXMESH_SYSTEMMEM, &pMeshHW, &pmcMesh->pMeshSW);
            if (FAILED(hr))
                goto e_ExitNONINDEXED;
            pmcMesh->pMesh->Release();
            pmcMesh->pMesh = pMeshHW;
            pMeshHW = NULL;
        }
		else
		{
...
e_ExitNONINDEXED:
    ...
    else if (m_method == D3DINDEXED)
    {
        ...
        if (caps.MaxVertexBlendMatrixIndex + 1 < maxFaceInfl)
        {
        ...
        {
            pmcMesh->m_paletteSize = min(caps.MaxVertexBlendMatrixIndex + 1, pmcMesh->m_pSkinMesh->GetNumBones());
            pmcMesh->m_bUseSW = false;
        }

        DWORD options = D3DXMESH_WRITEONLY | ((pmcMesh->m_bUseSW)? D3DXMESH_SYSTEMMEM : 0);
        hr = pmcMesh->m_pSkinMesh->ConvertToIndexedBlendedMesh(options, pAdjacencyIn, pmcMesh->m_paletteSize, NULL,
