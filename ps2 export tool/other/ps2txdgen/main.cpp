...
inline eTargetPlatform GetRasterPlatform( rw::Raster *texRaster )
{
    eTargetPlatform thePlatform = PLATFORM_UNKNOWN;

    if ( texRaster->hasNativeDataOfType( "Direct3D" ) )
    {
...
inline double GetPlatformQualityGrade( eTargetPlatform platform )
{
    double quality = 0.0;
...
inline bool ShouldRasterConvertBeforehand( rw::Raster *texRaster, eTargetPlatform targetPlatform )
{
...
inline void ConvertRasterToPlatform( rw::TextureBase *theTexture, rw::Raster *texRaster, eTargetPlatform targetPlatform, rw::KnownVersions::eGameVersion gameVersion )
{
    ...
    else if ( targetPlatform == PLATFORM_PC )
    {
        ...
        hasConversionSucceeded = couldConvert;
    }
    ...
    {
        rwEngine->PushWarning( "TxdGen: failed to convert texture " + theTexture->GetName() );
    }
}
                    ...
                    for ( rw::TexDictionary::texIter_t iter = txd->GetTextureIterator(); !iter.IsEnd(); iter.Increment() )
                    {
                        ...
                        if ( texRaster )
                        {
                            ...
                            if ( generateMipmaps )
                            {
                                ...
                                theTexture->generateMipmaps( mipGenMaxLevel + 1, mipGenMode );
                            }
