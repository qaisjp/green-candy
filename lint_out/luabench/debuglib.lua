        ...
        function(c)
            ...
            function addDebugProxy(name, ...)
                ...
                proxy.handler = function(...)
                    ...
                    while (n <= count) do
                        local arg = args[n];
                        local this_type = rawtype(arg);
                        local exp_type = argInfo[n];
                        local d_type = rawtype(exp_type);
            ...
            function setArgumentTypeCallback(typeName, callback)
                argTypeHandlers[typeName] = callback;
                return true;
            end
            
            -- Attempt to get a callback for said function.
            function getProxy(name, super)
