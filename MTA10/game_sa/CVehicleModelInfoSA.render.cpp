...
{
    return !( atomic->m_componentFlags & 0x0400 ) || *(float*)0x00C88020 < 0.2f || calc > 0.0f;
}
...
{
    return RwAtomicIsVisibleBasic( atomic, calc ) || !( atomic->m_componentFlags & 0x80 ) && ( calc * calc ) >= *(float*)VAR_ATOMIC_RENDER_OFFSET * 0.1f;
}
...
static RpAtomic* RwAtomicRenderHeliMovingRotor( RpAtomic *atomic )
{
    ...
    CVector vecRotor = atomic->m_parent->GetLTM().pos - *(CVector*)VAR_CameraPositionVector;
    RwAtomicZBufferEntry level;

    // Calculate rotor details
    level.m_distance = vecRotor.DotProduct( atomic->m_clump->m_parent->GetLTM().up ) * 20 + *(float*)VAR_ATOMIC_RENDER_OFFSET;
    level.m_render = RpAtomicRender;
    level.m_atomic = atomic;
...
static RpAtomic* RwAtomicRenderHeliMovingRotor2( RpAtomic *atomic )
{
    ...
    CVector vecRotor = atomic->m_parent->m_ltm.pos - *(CVector*)VAR_CameraPositionVector;
    RwAtomicZBufferEntry level;

    // Lulz, heavy math, much assembly, small C++ code
    level.m_distance = *(float*)VAR_ATOMIC_RENDER_OFFSET - vecRotor.DotProduct( atomic->m_clump->m_parent->GetLTM().right )
                                                         - vecRotor.DotProduct( atomic->m_clump->m_parent->GetLTM().at );
    level.m_render = RpAtomicRender;
    level.m_atomic = atomic;
