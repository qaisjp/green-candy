...
CPlayerPedSAInterface* __cdecl GetPlayerPed( unsigned int id )
{
    if ( id == 0xFFFFFFFF )
        id = *(unsigned char*)0x00B7CD74;

    return ((CPlayerInfoSAInterface*)CLASS_CPlayerInfo)[id].m_ped;
}
    ...
    for ( ActiveEntityNode *node = *(ActiveEntityNode**)0x00965500; node != (ActiveEntityNode*)0x009654F0; node = node->next )
    {
        ...
        if ( !*(bool*)0x009654BD )
        {
            if ( !checkOnScreen || finalLOD->IsOnScreen() )
            {
                ...
                if ( ( curArea == *(unsigned int*)0x00B72914 || curArea == 13 ) &&
