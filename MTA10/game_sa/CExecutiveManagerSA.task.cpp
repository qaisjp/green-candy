...
*  FILE:        game_sa/CExecutiveManagerSA.task.cpp
...
static AINLINE bool ProcessSheduleItem( void )
{
    ...
    if ( hasSheduledItem )
    {
        ...
        CExecTaskSA *theTask = currentSheduling.task;
...
static void __stdcall TaskFiberWrap( CFiberSA *theFiber, void *memPtr )
{
    // Initialize the runtime, as the pointer is only temporary.
    CExecTaskSA *theTask = NULL;
    void *userdata = NULL;
...
CExecTaskSA* CExecutiveManagerSA::CreateTask( CExecTaskSA::taskexec_t callback, void *userdata, size_t stackSize )
{
    ...
    CExecTaskSA *task = new CExecTaskSA( this, theFiber );
...
void CExecutiveManagerSA::CloseTask( CExecTaskSA *task )
{
...
void CExecTaskSA::WaitForFinish( void )
{
    CFiberSA *theFiber = runtimeFiber;
