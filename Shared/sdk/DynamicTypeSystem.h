...
struct DynamicTypeSystem
{
    ...
    struct pluginDescriptor
    {
        ...
        AINLINE pluginStructType* RESOLVE_STRUCT( GenericRTTI *object, pluginOffset_t offset )
        {
            return DynamicTypeSystem::RESOLVE_STRUCT <pluginStructType> ( object, this->typeInfo, offset );
        }

        template <typename pluginStructType>
        AINLINE const pluginStructType* RESOLVE_STRUCT( const GenericRTTI *object, pluginOffset_t offset )
        {
            return DynamicTypeSystem::RESOLVE_STRUCT <const pluginStructType> ( object, this->typeInfo, offset );
        }
    };
    ...
    AINLINE static pluginOffset_t GetTypeInfoStructOffset( GenericRTTI *rtObj, typeInfoBase *offsetInfo )
    {
    ...
    template <typename pluginStructType>
    AINLINE static pluginStructType* RESOLVE_STRUCT( GenericRTTI *rtObj, typeInfoBase *typeInfo, pluginOffset_t offset )
    {
    ...
    template <typename pluginStructType>
    AINLINE static const pluginStructType* RESOLVE_STRUCT( const GenericRTTI *rtObj, typeInfoBase *typeInfo, pluginOffset_t offset )
    {
        if ( !IsOffsetValid( offset ) )
            return NULL;

        size_t baseOffset = GetTypeInfoStructOffset( (GenericRTTI*)rtObj, typeInfo );
    ...
    typedef CommonPluginSystemDispatch <GenericRTTI, DynamicTypeSystem, pluginDescriptor> functoidHelper_t;
    ...
    inline bool ConstructPlugins( typeInfoBase *typeInfo, GenericRTTI *rtObj )
    {
        ...
        {
            parentConstructionSuccess = ConstructPlugins( inheritedClass, rtObj );
        }

        // If the parents have constructed properly, do ourselves.
        if ( parentConstructionSuccess )
        {
            bool thisConstructionSuccess = typeInfo->structRegistry.ConstructPluginBlock( rtObj );
    ...
    inline bool AssignPlugins( typeInfoBase *typeInfo, GenericRTTI *dstRtObj, const GenericRTTI *srcRtObj )
    {
        ...
        {
            parentAssignmentSuccess = AssignPlugins( inheritedType, dstRtObj, srcRtObj );
        }

        // If all the parents successfully assigned, we can try assigning ourselves.
        if ( parentAssignmentSuccess )
        {
            bool thisAssignmentSuccess = typeInfo->structRegistry.AssignPluginBlock( dstRtObj, srcRtObj );
    ...
    inline void DestructPlugins( typeInfoBase *typeInfo, GenericRTTI *rtObj )
    {
        try
        {
    ...
    inline GenericRTTI* ClonePlacement( void *objMem, const GenericRTTI *toBeCloned )
    {
        GenericRTTI *objOut = NULL;
        {
            ...
            if ( objStruct )
            {
                ...
                if ( pluginConstructSuccess )
                {
                    ...
                    bool pluginAssignSuccess = AssignPlugins( typeInfo, objTypeMeta, toBeCloned );
        ...
        GenericRTTI *objOut = NULL;
        {
            ...
            if ( objMemSize != 0 )
            {
                ...
                if ( objMem )
                {
                    ...
                    objOut = ClonePlacement( objMem, toBeCloned );
                }
...
public:
    ...
    inline void DestroyPlacement( GenericRTTI *typeStruct )
    {
